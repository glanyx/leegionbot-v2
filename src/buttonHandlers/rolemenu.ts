...
export class Rolemenu extends ButtonHandler {
  ...
  public static async execute({
    ...
    if (!id || !guild || !member) return interaction.reply('Unable to edit role at this time.').catch(e => {
    ...
    if (!role) return interaction.reply('Unable to edit role at this time.').catch(e => {
      logger.debug(e.message)
