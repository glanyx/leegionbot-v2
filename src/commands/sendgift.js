...
export async function run (client, message, args) {
    ...
    try{
        while (!gift.confirmed) {
            ...
            gift.confirmed = await confirmInput(santa, buildEmbed(gift, client.user), `This is the message that will be sent to **${recipient.username}**. Is this correct?`);
        }
        santa.send(`Thanks ${santa}! **${recipient.username}** should receive their gift in just a moment!`);
...
function getType(user){
    return new Promise(
        (resolve, reject) => {
            getInput(user, "Awesome! And what is it I'm sending? Am I sending a gift code? Am I sending a link to artwork?")
            .then(collected => {
                resolve(collected);
            })
            .catch(e => {
...
function checkMessage(user){
    return new Promise(
        (resolve, reject) => {
            confirmInput(user, "Do you want to send a message along with your gift?\n**PROTIP: This is a great chance to wish your TARGET a Merry Christmas!**")
            .then(collected => {
                resolve(collected);
            })
            .catch(e => {
    ...
    return new Promise(
        (resolve, reject) => {
            ...
            .then(async message => {
                ...
                message.awaitReactions((reaction, userA) => userA === user, {
                    max: 1,
                    time: 180000,
                    errors: ['time']
                }).then(collected => {
...
function buildEmbed(profile, bot){
    ...
    let embed = new RichEmbed()
        .setImage('https://img.etimg.com/thumb/msid-67267294,width-643,imgsize-671579,resizemode-4/santaa.jpg')
        .setColor('#d42426')
        ...
        async (resolve, reject) => {
            ...
            try{
                ...
                if (profile.Item){
                    ...
                    reject(new Error(`I couldn't find your target, ${user}! Please try again, or if you keep running into this issue, contact my owner.`))
                    console.log(`No target for ${user.id}!`);
                    return;
                }
                
            } catch (e) {
                reject(new Error(`I couldn't find you in my records! Are you sure you're a Secret Santa?`));
                console.log(e);
            }
