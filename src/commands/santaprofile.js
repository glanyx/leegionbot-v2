...
exports.run = async (client, message, args) => {
    ...
    if (application.owner === message.author) {
      if (args.length > 0) {
        try {
          ...
          message.author.send(
            `Couldn't find the specified user. Error logged to console.`
          );
          console.log(e);
          return;
        }
  ...
  try {
    ...
    if (profile.Item) {
      ...
      message.author.send(
        `It doesn't look like you've registered yet! You can register for the Secret Santa event by typing \`${config.prefix}santaregister\`!`
      );
      return;
    }
  } catch (e) {
    message.author.send(
      `I couldn't fetch your profile! Please try again later or contact my owner.`
    );
