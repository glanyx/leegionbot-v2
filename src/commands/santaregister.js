...
exports.run = async (client, message, args) => {
  ...
  try {
    ...
    message.author.send(
      `The Secret Santa registration process has been cancelled. If you still want to register, please use the \`santaregister\` command again!`
    );
  }
};
...
async function termsAndConditions(message) {
  ...
  return new Promise(async (resolve, reject) => {
    await user
      ...
      .catch(e => {
        message.channel.send(
          `I can't DM you, ${message.author}! If you have DMs disabled, please temporarily enable them!`
        );
    ...
    user
      .send({
        embed: {
          ...
          }, created by ${await user.client
            .fetchUser("224606487979360256")
            .then(user => {
              return user.tag;
            })}, will hereby be approved to receive publically available data from your Discord Profile and any additional information that you send it. Aforementioned data is stored on an AWS server in London and will only be used for the sole purpose of the Leegion Secret Santa event. Your data will only be shared with the bot owner and any Discord user that you are matched with during the event.\n
                **Your data will never be shared with, or sold to additional third parties.**\n
                Neither ${
                  user.client.user.username
                }, its owner ${await user.client
            .fetchUser("224606487979360256")
            .then(user => {
              return user.tag;
            })}, the Leegion Mod team, nor Amanda Lee (known as LeeandLie or Amalee) can be held responsible for any missing items / gifts or any potential unfair balance between gifts sent or gifts received.\n
      ...
      .then(async message => {
        ...
        message
          .awaitReactions(
            (reaction, userA) =>
              (reaction.emoji.id === "507285695484919809" ||
                reaction.emoji.id === "507287289282428962") &&
              userA === user,
            {
              max: 1,
              time: 180000,
...
async function type(user) {
  return new Promise(async (resolve, reject) => {
    ...
    user
      .send(
        `Please type either \`free\`, \`digital\` or \`physical\`, based on the tier you would like to opt in for.`
      )
      .then(message => {
...
async function getLocation(user) {
  return new Promise(async (resolve, reject) => {
    user
      .send(
        `Nice! Now that I've got you written down for the Physical tier, I need to ask you for your physical address so that you may receive a gift from your Secret Santa, exciting! Please type your FULL address to me the same way as you would put on a letter.\nHint: You can use the Shift + Enter keys on your keyboard to create a new line without sending me your message!`
      )
      .then(message => {
        const collector = message.channel.createMessageCollector(
          message => user === message.author,
...
async function getRegion(user) {
  return new Promise(async (resolve, reject) => {
    user
      .send(
        `To match you with a Secret Santa, I'll need a rough region of where you live. I'm not smart enough (yet) to figure this out myself, so please give me a hand!\nThe options are:
            Europe
            North America
            South America
            Canada
      ...
      .then(message => {
        ...
        async function regionConfirm(user, region) {
          ...
          await confirmInput(
            user,
            regionArray[region],
            `You have chosen the following Region. Is this correct?`
          )
            .then(() => {
              collector.stop();
              resolve(region);
        ...
        collector.on("collect", async m => {
          if (
            m.content.toLowerCase().includes("europe") ||
            m.content.toLowerCase() === "eu"
          ) {
            await regionConfirm(user, `eu`);
          } else if (
            m.content.toLowerCase().includes("north america") ||
            m.content.toLowerCase() === "na" ||
            m.content.toLowerCase().includes("usa")
          ) {
            await regionConfirm(user, `na`);
          } else if (
            m.content.toLowerCase().includes("south america") ||
            m.content.toLowerCase() === "sa"
          ) {
            await regionConfirm(user, `sa`);
          } else if (
            m.content.toLowerCase().includes("canada") ||
            m.content.toLowerCase() === "ca"
          ) {
            await regionConfirm(user, `ca`);
          } else if (
            m.content.toLowerCase().includes("asia") ||
            m.content.toLowerCase() === "as"
          ) {
            await regionConfirm(user, `as`);
          } else if (
            m.content.toLowerCase().includes("australia") ||
            m.content.toLowerCase().includes("aus") ||
            m.content.toLowerCase() === "au"
          ) {
            await regionConfirm(user, `au`);
          } else if (
            m.content.toLowerCase().includes("middle east") ||
            m.content.toLowerCase() === "me"
          ) {
            await regionConfirm(user, `me`);
          } else if (
            m.content.toLowerCase().includes("africa") ||
            m.content.toLowerCase() === "af"
          ) {
    ...
    user
      ...
      .then(async message => {
        ...
        message
          .awaitReactions(
            (reaction, userA) =>
              reaction.emoji.id === "507285695484919809" && userA === user,
            {
              max: 1,
              time: 180000,
              errors: ["time"]
